...
# Comprehensive Latency Analysis Script for ULTRA-OPTIMIZED 2 Workers
# Provides detailed statistics up to 99.99 percentile with comparisons

set -e

OUTPUT_FILE="high-volume-latency-results-2workers-optimized.jsonl"
TEMP_LATENCIES="/tmp/latencies_sorted_2workers_optimized.txt"

if [ ! -f "$OUTPUT_FILE" ]; then
    echo "❌ Error: Results file '$OUTPUT_FILE' not found"
    echo "🔧 Run './high-volume-test-2workers-optimized.sh' first to generate test results"
    exit 1
fi

echo "📊 High-Volume Latency Analysis - ULTRA-OPTIMIZED 2 WORKERS"
echo "══════════════════════════════════════════════════════════════"
echo ""
...
if [ $FIRST_EPOCH -ne 0 ] && [ $LAST_EPOCH -ne 0 ] && [ $LAST_EPOCH -gt $FIRST_EPOCH ]; then
    ...
    IMPROVEMENT_SINGLE=$(echo "scale=2; ($ACTUAL_THROUGHPUT / 518.43) * 100" | bc -l)
    IMPROVEMENT_STANDARD=$(echo "scale=2; ($ACTUAL_THROUGHPUT / 526.93) * 100" | bc -l)
    echo "   • Test duration: ${DURATION} seconds"
    echo "   • Actual throughput: ${ACTUAL_THROUGHPUT} msg/s"
    echo "   • Target throughput: 2,000 msg/s"
    echo "   • Improvement over 1 worker: ${IMPROVEMENT_SINGLE}%"
    echo "   • Improvement over standard 2 workers: ${IMPROVEMENT_STANDARD}%"
else
...
echo "   • Total workers: 2 (ultra-optimized)"
echo "   • Messages per worker (avg): $(echo "scale=0; $TOTAL_MESSAGES / 2" | bc -l)"
echo "   • Optimizations applied:"
echo "     - Enhanced fetch settings (5ms wait vs 10ms)"
echo "     - Sticky partition assignment"
echo "     - Larger producer batches with micro-linger"
echo "     - Optimized connection management"
echo "     - Reduced retry delays"
echo ""
...
CSV_OUTPUT="high-volume-latency-stats-2workers-optimized.csv"
...
echo "optimization_level,ultra_optimized,string" >> $CSV_OUTPUT
...
echo "📊 Key insights (ULTRA-OPTIMIZED 2 workers):"
...
if [ $FIRST_EPOCH -ne 0 ] && [ $LAST_EPOCH -ne 0 ] && [ $LAST_EPOCH -gt $FIRST_EPOCH ]; then
    echo "   • Throughput improvement: ${IMPROVEMENT_STANDARD}% over standard 2 workers"
fi
echo ""
...
# Comprehensive comparison with all previous tests
echo ""
...
if [ -f "high-volume-latency-stats.csv" ]; then
    ...
    SINGLE_THROUGHPUT=$(grep "actual_throughput" high-volume-latency-stats.csv | cut -d',' -f2 2>/dev/null || echo "518.43")
    
    echo "   📊 Single Worker vs Ultra-Optimized 2 Workers:"
    echo "      • P99.99: ${P99_99}ms vs ${SINGLE_P99_99}ms"
    echo "      • P99:    ${P99}ms vs ${SINGLE_P99}ms"
    echo "      • P50:    ${P50}ms vs ${SINGLE_P50}ms"
    echo "      • Throughput: ${ACTUAL_THROUGHPUT} vs ${SINGLE_THROUGHPUT} msg/s"
    echo ""
fi

if [ -f "high-volume-latency-stats-2workers.csv" ]; then
    STANDARD_P99_99=$(grep "p99_99_latency" high-volume-latency-stats-2workers.csv | cut -d',' -f2)
    STANDARD_P99=$(grep "p99_latency" high-volume-latency-stats-2workers.csv | cut -d',' -f2)
    STANDARD_P50=$(grep "p50_latency" high-volume-latency-stats-2workers.csv | cut -d',' -f2)
    STANDARD_THROUGHPUT=$(grep "actual_throughput" high-volume-latency-stats-2workers.csv | cut -d',' -f2 2>/dev/null || echo "526.93")
    
    echo "   📊 Standard 2 Workers vs Ultra-Optimized 2 Workers:"
    echo "      • P99.99: ${P99_99}ms vs ${STANDARD_P99_99}ms"
    echo "      • P99:    ${P99}ms vs ${STANDARD_P99}ms"
    echo "      • P50:    ${P50}ms vs ${STANDARD_P50}ms"
    echo "      • Throughput: ${ACTUAL_THROUGHPUT} vs ${STANDARD_THROUGHPUT} msg/s"
    echo ""
fi
...
echo "───────────────────────"
echo "   • Configuration: Ultra-optimized 2-worker setup"
echo "   • Key optimizations:"
echo "     - Sticky partition balancing"
echo "     - Enhanced fetch settings"
echo "     - Producer micro-batching"
echo "     - Optimized connection management"
echo "   • Result: Balanced throughput and latency performance"
