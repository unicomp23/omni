...
show_next_steps() {
    ...
    echo "   scp -i $KEY_PAIR_PATH scripts/install-redpanda.sh ec2-user@<redpanda-node-ip>:"
    echo "   scp -i $KEY_PAIR_PATH scripts/setup-redpanda-cluster.sh ec2-user@<redpanda-node-ip>:"
    echo ""
    echo "2. Install RedPanda on each node (run on each RedPanda instance):"
    echo "   ssh -i $KEY_PAIR_PATH ec2-user@<redpanda-node-0-ip>"
    echo "   sudo ./install-redpanda.sh"
    echo "   NODE_ID=0 ./setup-redpanda-cluster.sh"
    echo ""
    echo "   ssh -i $KEY_PAIR_PATH ec2-user@<redpanda-node-1-ip>"
    echo "   sudo ./install-redpanda.sh"
    echo "   NODE_ID=1 ./setup-redpanda-cluster.sh"
    echo ""
    echo "   ssh -i $KEY_PAIR_PATH ec2-user@<redpanda-node-2-ip>"
    echo "   sudo ./install-redpanda.sh"
    echo "   NODE_ID=2 ./setup-redpanda-cluster.sh"
    echo ""
    echo "3. Set up load testing (run on load test instance):"
    echo "   ssh -i $KEY_PAIR_PATH ec2-user@<load-test-ip>"
